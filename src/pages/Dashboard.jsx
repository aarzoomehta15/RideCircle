...
//         feedbackService.getFeedbackForUser(user._id),
//       ]);
//       setPools(poolsData.pools || []);
...
const Dashboard = ({ user }) => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // ✅ Fetch pools created by YOU 
      // ✅ Fetch feedback RECEIVED by YOU, not the ones you gave
      const [poolsData, feedbackData] = await Promise.all([
        poolService.getMyPools(),
        feedbackService.getFeedbackForUser(user._id), // <-- FIXED
      ]);
  ...
  // ✅ Compute AVG rating received by user (not given by user)
  const avgRating =
    feedbacks.length > 0
      ? (
  ...
  return (
    <div className="dashboard">
      ...
      <div className="stats">
        <p><strong>Total Pools Created:</strong> {pools.length}</p>
