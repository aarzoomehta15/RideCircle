...
const CreatePool = () => {
  ...
  // Logic to determine available pool types and default value
  const isMale = user?.gender === 'male';
  const hasCommunity = user?.community?.length > 0;

  const getAvailablePoolTypes = () => {
    let types = [
      { value: "open", label: "Open to All" },
      { value: "community", label: "Community Members Only", disabled: !hasCommunity },
    ];
    
    if (!isMale) {
      types.push({ value: "women-only", label: "Women Only" });
    }
    
    return types;
  };

  const availablePoolTypes = getAvailablePoolTypes();
  const defaultPoolType = availablePoolTypes.find(type => !type.disabled)?.value || 'open';
  ...
  const handleChange = (e) => {
    ...
    // Logic to prevent selecting a disabled option, though UI should hide it
    if (name === 'type' && value === 'community' && !hasCommunity) {
      return;
    }
  ...
  const handleSourceLocation = useCallback((location) => {
  ...
  const handleDestLocation = useCallback((location) => {
    setFormData((prevData) => ({
      ...prevData,
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.type === 'community' && !hasCommunity) {
        setError("You must belong to a community to create a 'Community Members Only' pool.");
        return;
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                {formData.type === 'community' && !hasCommunity && (
                    <p className="mt-2 text-sm text-red-600">You must add a community to your profile to use this option.</p>
                )}
              </div>
